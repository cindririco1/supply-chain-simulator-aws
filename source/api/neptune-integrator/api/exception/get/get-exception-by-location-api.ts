...
export class GetExceptionByLocationApi implements ApiInterface {
    logger: Logger;
    ...
    async execute(path: string, body: string | null, _headers: object): Promise<ApiResponse> {
        ...
        const locationId = PathParser.getLastItemInPath(path, GetExceptionByLocationApi.EXPECTED_PATH_PARTS);

        const results = await this.db.getInvalidItems(locationId, undefined);
