...
import { GetExceptionByItemApi } from "../../../../api/exception/get/get-exception-by-item-api";
...
describe("GetExceptionByItemApi", () => {
    it("should construct", async () => {
        // arrange
        const db = NeptuneDB.prototype;

        // act
        const api = new GetExceptionByItemApi(db);
    ...
    it("should return proper values", async () => {
        ...
        // act
        const result = await api.execute(`GET /exception/item/${itemId}`, "", {});

        // assert
        expect(mock).toHaveBeenCalledTimes(1);
        expect(mock).toHaveBeenCalledWith(undefined, itemId);
